...
PACKAGE ?= aws-actions-connector
XRD_DIR := apis/actionsconnectors
COMPOSITION := $(XRD_DIR)/composition.yaml
DEFINITION := $(XRD_DIR)/definition.yaml
EXAMPLE_DEFAULT := examples/actionsconnectors/standard.yaml
...
EXAMPLES := \
    examples/actionsconnectors/standard.yaml:: \
    examples/actionsconnectors/single-repo.yaml:: \
    examples/actionsconnectors/environment-deploy.yaml:: \
    examples/actionsconnectors/import-existing.yaml::
...
render\:%:
	@example="examples/actionsconnectors/$$*.yaml"; \
	if [ -f "$$example" ]; then \
		echo "=== Rendering $$example ==="; \
		up composition render --xrd=$(DEFINITION) $(COMPOSITION) $$example; \
	else \
